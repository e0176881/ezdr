     ...
     /*   TextView loginScreen = (TextView) findViewById(R.id.link_to_login);
        ...
        b.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(nric.getText().toString().length() == 0)
				{
					showAlert2("Missing NRIC field");
					
				}
				else if(fullname.getText().toString().length() == 0)
				{
					
					showAlert2("Missing fullname field");
				}
				else if(address.getText().toString().length() == 0)
				{
					
					showAlert2("Missing address field");
				}
				else if(allergies.getText().toString().length() == 0)
				{
					
					showAlert2("Missing allergies field");
				}
				else if(medication.getText().toString().length() == 0)
				{
					
					showAlert2("Missing medication field");
				}
				else if(medicalhistory.getText().toString().length() == 0)
				{
					
					showAlert2("Missing medicalhistory field");
				}
				else
				{
			
				dialog = ProgressDialog.show(RHistory.this, "", 
                        "Updating...", true);
 ...
 slidingimage.setScaleType(ScaleType.FIT_XY);
    ...
    void Register(){
  			...
  			if(response.contains("success")){
  				runOnUiThread(new Runnable() {
  				    public void run() {
  				    	...
  				    	Toast.makeText(RHistory.this,"History Updated!", Toast.LENGTH_SHORT).show();
  				    	startActivity(new Intent(RHistory.this, skiplogin.class));
  				    }
  				});
  	...
  	public void showAlert(){
  		RHistory.this.runOnUiThread(new Runnable() {
  		    public void run() {
  		    	AlertDialog.Builder builder = new AlertDialog.Builder(RHistory.this);
  	...
  	public void showAlert2(final String a){
  		RHistory.this.runOnUiThread(new Runnable() {
		    public void run() {
		    	AlertDialog.Builder builder = new AlertDialog.Builder(RHistory.this);
		    	builder.setTitle("Error.");
